...
def login():
    if request.method == 'POST':
        ...
        print(f"登录尝试: 用户={username}, MFA令牌={mfa_token}")  # 调试日志
        
        user = auth_service.authenticate_user(username, password)
        if user:
            print(f"密码验证成功，MFA状态: enabled={user.mfa_enabled}, secret={user.mfa_secret}")  # 调试日志
            if user.mfa_enabled:
                if mfa_token:
                    print(f"验证MFA: 用户={username}, 令牌={mfa_token}")  # 调试日志
                    # 先验证MFA
                    if auth_service.verify_mfa(username, mfa_token):
                        print("MFA验证成功，准备登录用户")  # 调试日志
                        # MFA验证成功后再登录
                        user.mfa_verified = True
                        login_user(user)
                        print(f"用户登录状态: {current_user.is_authenticated}")  # 调试日志
