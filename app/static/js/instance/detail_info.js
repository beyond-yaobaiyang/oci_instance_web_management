...
let pollingTimer = 45000;
let vnicPollingInterval = 60000;
const POLLING_INTERVAL = 45000; // 45秒轮询一次
...
function updateInstanceInfo(instance) {
    ...
    // IPv6地址列表
    const ipv6Container = document.getElementById('instance-ipv6-addresses');
    if (instance.ipv6_addresses && instance.ipv6_addresses.length > 0) {
        const ipv6List = instance.ipv6_addresses.flat();  // 展平嵌套数组
        ipv6Container.innerHTML = ipv6List.map(ip => `
            <div class="badge bg-secondary me-1 mb-1">
                ${ip}
                <button type="button" class="btn-close btn-close-white ms-1" style="font-size: 0.5em;" 
                    onclick="deleteIpv6Address('${ip}')" aria-label="删除"></button>
            </div>
        `).join('');
    } else {
        ipv6Container.textContent = '-';
    }
...
async function showShapeModal() {
    const shapeModalElement = document.getElementById('shapeModal');
    if (!shapeModalElement) {
        showToast('找不到形状调整对话框', 'error');
        return;
    }
    
    const modal = new bootstrap.Modal(shapeModalElement);
    const currentShapeElement = document.getElementById('instance-shape');
    const shapeSelect = document.getElementById('shapeSelect');
    const flexShapeConfig = document.getElementById('flexShapeConfig');
    
    // 检查必要的元素是否存在
    if (!shapeSelect || !flexShapeConfig) {
        showToast('找不到必要的表单元素', 'error');
        return;
    }
    
    const currentShape = currentShapeElement ? currentShapeElement.textContent : '';
    ...
    try {
        ...
        console.error('获取可用实例形状失败:', error);
...
function showAddIpv6Modal() {
    ...
    fetch(`/instance/api/instance/${tenantId}/${instanceId}/vnics`)
        ...
        .then(vnics => {
            ...
            vnicSelect.innerHTML = '<option value="">请选择VNIC</option>';
            vnics.forEach(vnic => {
                const option = document.createElement('option');
                option.value = vnic.id;
                option.textContent = `${vnic.display_name || 'VNIC'} (${vnic.private_ip})`;
                vnicSelect.appendChild(option);
            });
            
            // 显示模态框
            const modal = new bootstrap.Modal(document.getElementById('ipv6Modal'));
            modal.show();
        })
        .catch(error => {
            showToast('获取VNIC列表失败: ' + error.message, 'error');
        });
}

// 监听IPv6类型选择变化
document.getElementById('ipv6Type').addEventListener('change', function() {
    const manualContainer = document.getElementById('manualIpv6Container');
    manualContainer.style.display = this.value === 'manual' ? 'block' : 'none';
});

function addIpv6Address() {
    const vnicId = document.getElementById('vnicSelect').value;
    const ipv6Type = document.getElementById('ipv6Type').value;
    const manualIpv6 = document.getElementById('manualIpv6').value;

    if (!vnicId) {
        showToast('请选择VNIC', 'warning');
        return;
    }

    const data = {
        vnic_id: vnicId,
        is_auto: ipv6Type === 'auto'
    };

    if (ipv6Type === 'manual') {
        if (!manualIpv6) {
            showToast('请输入IPv6地址', 'warning');
            return;
        }
        data.ipv6_address = manualIpv6;
    }

    fetch(`/instance/api/instance/${tenantId}/${instanceId}/ipv6`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
    ...
    .then(result => {
        ...
        showToast('IPv6地址添加成功');
        bootstrap.Modal.getInstance(document.getElementById('ipv6Modal')).hide();
        loadInstanceDetail();  // 刷新实例详情
    })
    .catch(error => {
        showToast('添加IPv6地址失败: ' + error.message, 'error');
    });
}

function deleteIpv6Address(ipv6Address) {
    showConfirmModal('删除IPv6地址', `确定要删除IPv6地址 ${ipv6Address} 吗？`)
        .then(confirmed => {
            if (!confirmed) return;
            ...
            fetch(`/instance/api/instance/${tenantId}/${instanceId}/ipv6`, {
                ...
                body: JSON.stringify({ ipv6_address: ipv6Address })
            })
            .then(response => response.json())
            .then(result => {
                if (result.error) {
                ...
                showToast('IPv6地址删除成功');
                loadInstanceDetail();  // 刷新实例详情
            })
            .catch(error => {
                showToast('删除IPv6地址失败: ' + error.message, 'error');
            });
        });
}