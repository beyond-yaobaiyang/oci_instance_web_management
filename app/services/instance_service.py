    ...
    def get_instance(self, tenant_id, instance_id):
        ...
        try:
            ...
            return {
                ...
                'ipv6_addresses': [vnic['ipv6_addresses'] for vnic in vnics if vnic['ipv6_addresses']]
            }
        except Exception as e:
...
"""
                ...
                try:
                    ...
                    try:
                        ...
                        vnics.append({
                            ...
                            'ipv6_addresses': vnic.ipv6_addresses if hasattr(vnic, 'ipv6_addresses') else [],
    ...
    def add_ipv6_address(self, tenant_id: str, instance_id: str, vnic_id: str, is_auto: bool = True, ipv6_address: str = None) -> bool:
        """
        添加IPv6地址到VNIC
        :param tenant_id: 租户ID
        :param instance_id: 实例ID
        :param vnic_id: VNIC ID
        :param is_auto: 是否自动分配IPv6地址
        :param ipv6_address: 手动指定的IPv6地址（仅在is_auto为False时使用）
        :return: bool 是否成功
        """
        try:
            network_client = self._get_network_client(tenant_id)
            if not network_client:
                raise Exception("无法创建网络客户端")
            ...
            if not is_auto and ipv6_address:
                create_ipv6_details.ip_address = ipv6_address

            # 创建IPv6地址
            response = network_client.create_ipv6(
                create_ipv6_details=create_ipv6_details
            )
            ...
            try:
                oci.wait_until(
                    network_client,
                    network_client.get_ipv6(response.data.id),
                    'lifecycle_state',
                    'AVAILABLE',
                    max_wait_seconds=300
                )
                ...
                logging.error(f"等待IPv6创建完成时出错: {str(e)}")
                return False
    ...
    def delete_ipv6_address(self, tenant_id: str, instance_id: str, ipv6_address: str) -> bool:
        """
        删除IPv6地址
        :param tenant_id: 租户ID
        :param instance_id: 实例ID
        :param ipv6_address: 要删除的IPv6地址
        :return: bool 是否成功
        """
        try:
            network_client = self._get_network_client(tenant_id)
            ...
            target_vnic = None
            for vnic in vnics:
                if ipv6_address in vnic.get('ipv6_addresses', []):
                    target_vnic = vnic
                    break

            if not target_vnic:
                raise Exception(f"找不到包含IPv6地址 {ipv6_address} 的VNIC")

            # 获取VNIC的IPv6地址列表
            ipv6s = network_client.list_ipv6s(
                vnic_id=target_vnic['id']
            ).data

            # 找到匹配的IPv6地址
            target_ipv6 = None
            for ipv6 in ipv6s:
                if ipv6.ip_address == ipv6_address:
                    target_ipv6 = ipv6
                    break

            if not target_ipv6:
                raise Exception(f"找不到IPv6地址: {ipv6_address}")

            # 删除IPv6地址
            response = network_client.delete_ipv6(target_ipv6.id)
            
            # 等待IPv6删除完成
            try:
                oci.wait_until(
                    network_client,
                    network_client.get_ipv6(target_ipv6.id),
                    'lifecycle_state',
                    'TERMINATED',
                    max_wait_seconds=300,
                    succeed_on_not_found=True
                ...
                if e.status == 404:  # 资源已经被删除
                    return True
                raise
            except Exception as e:
                logging.error(f"等待IPv6删除完成时出错: {str(e)}")
                return False
